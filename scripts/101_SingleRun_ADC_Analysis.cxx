...
constexpr double adc_sum_bin_width = 300.0;

std::pair<int, double> adc_sum_histogram_axis(const std::vector<double>& sums)
{
	const auto max_sum = sums.empty() ? 0.0 : *std::max_element(sums.begin(), sums.end());
	const auto bin_count = std::max(1, static_cast<int>(std::ceil(max_sum * 1.05 / adc_sum_bin_width)));
	return {bin_count, bin_count * adc_sum_bin_width};
}
...
void draw_adc_sum_histogram_pages(const std::vector<double>& sums, const std::string& output_path, const std::string& name, const std::string& title, const PlotContext& context)
{
	...
	const auto [bin_count, x_max] = adc_sum_histogram_axis(sums);
	TH1D hist(("h_" + name).c_str(), (title + ";#Sigma_{channels}(max(val0)-pedestal);events").c_str(), bin_count, 0.0, x_max);
...
{
	const auto [bin_count, x_max] = adc_sum_histogram_axis(sums);
	auto hist = std::make_unique<TH1D>(name.c_str(), (title + ";#Sigma_{channels}(max(val0)-pedestal);events").c_str(), bin_count, 0.0, x_max);
	for (const auto sum : sums) {
		hist->Fill(sum);
	}
